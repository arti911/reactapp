...
import { ITickets, SearchIdResponse } from "./index.interface";
...
const Aviasales = () => {
  ...
  const [stops, setStops] = useState<CheckboxValueType[]>(Object.keys(STOPS));

  useEffect(() => {
    getSearchId<SearchIdResponse>()
      .then((response) => {
        if (response) {
          setSearchId(response.searchId);
        }
      })
      .catch(() => message.warning("Что-то пошло не так, перезагрузите страницу"));
  }, []);
  ...
  }, [defaultFilter, endSearchTickets, firstPackTickets, searchId, stops, ticketsAll]);
  ...
  const onLoadTickets = (): void => {
    setSortTickets({
      ...
      viewTickets: [
        ...sortTickets.viewTickets,
        ...sortTickets.tickets.slice().splice(sortTickets.viewTickets.length, LIMIT),
      ],
    });
  };
          ...
          <Tabs defaultActiveKey={defaultFilter} onChange={onChangeTabs}>
            {TABS.map((tab) => (
              <TabPane tab={tab.title} key={tab.key}>
                ...
                {stops.length === 0 && (
                  <Empty
                    description={`Мы нашли ${sortTickets.tickets.length} билетов по выбраным фильтрам`}
                  />
                )}
