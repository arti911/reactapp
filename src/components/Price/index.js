...
const Price = () => {
    ...
    useEffect(() => {
        getExchangeRates();
    }, [query, currency, getExchangeRates]);

    const getExchangeRates = async () => {
        const response = await fetch(
            `https://api.coindesk.com/v1/bpi/currentprice/${query}.json`
        );
        ...
        <div className="price">
            ...
            >
                {countries.map((country) => (
                    <option value={country.currency} key={country.currency}>
                        {country.country}
                    </option>
                ))}
            </select>
            <div className="price__container">
                {currency.map((curr) => (
                    <Currency data={curr} key={curr.code} />
                ))}
            </div>
        </div>
    );
